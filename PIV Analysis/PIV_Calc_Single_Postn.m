%% CALCULATES THE FINAL PIV IMAGES BY FILTERING BAD VECTORS ACROSS IMAGES FOR ONE CAMERA POSITION
...
cd([root1 'Jet_Analysis\Global_Functions\']); configTable;
disp([newline,'  Vector Calculation for Bottom position of Cameras']);       
...
%  LOADING DATA
load([driveIn 'U_vel']);     load([driveIn 'V_vel']);  % load([drive_in 'X']);         load([drive_in 'Y']);
load([driveIn 'X_loc']);     load([driveIn 'Y_loc']);  img_no = size(U_vel,3);
Flow = zeros(size(U_vel,1),size(U_vel,2),6);   InputStruct.config = config;
%% LIMITS ON U AND V VELOCITY FOR IMAGE FILTERING FOR CAMERA 1(C1) & CAMERA 2(C2) FOR EACH POSITION(D/U)
InputStruct.Upos_C1 = 2.5;   InputStruct.Vpos_C1 = 16;     InputStruct.Vneg_C1 = 16;   StdDev_Cutoff = 2.5;
InputStruct.Upos_C2 = NaN;   InputStruct.Vpos_C2 = NaN;    InputStruct.Vneg_C2 = NaN;  clc;
%  FILTERING & COMPUTING FLOW METRICS
pos = 1;
%  ADDING OTHER INPUTS TO STRUCT FOR SORTING
InputStruct.U = U_vel(:,:,:,pos);   InputStruct.V = V_vel(:,:,:,pos); InputStruct.n_cam = pos;
...
subplot(131);  plot(U_Imgset);   xlim([0 img_no]);    title(['Ux peaks. Camera - ',num2str(m)]);    box off;  grid on;  hold on; 
%  PLOTTING THE IMAGE CUTOFF FOR +ve V - VELOCITY
subplot(132); plot(Vpos_imgset);   xlim([0 img_no]);  title(['Vy +ve peaks. Camera - ',num2str(m)]);box off;  grid on;
%  PLOTTING THE IAMGE CUTOFF FOR -ve V - VELOCITY
subplot(133); plot(Vneg_imgset);   xlim([0 img_no]);  title(['Vy -ve peaks. Camera - ',num2str(m)]);box off;  grid on;
...
%  HIGHER ORDER STATISTICS NORMALIZATION
...
%  REYNOLD'S STRESSES
Rxy = (1/(size(U_prim,3)-1)) * mean(U_prim.*V_prim,3);
%  STORING STATISTICS
...
U_bar = nanmean(Master_U,3);            V_bar = mean(Master_V,3);
%  FLUCTUATING VELOCITIES
U_prim = Master_U(:,:,:) - U_bar;       V_prim = Master_V(:,:,:) - V_bar;
U_primsq = (mean((U_prim).^2,3))/Uj^2;  V_primsq = (mean((V_prim).^2,3))/Uj^2;
% AXES DEFINITION
...
colorbar;      xlim([min(X) max(X)]);  ylim([min(Y) max(Y)]);
toc
%% WRITING TO FILE
prompt = ([newline '->> Save Data to Disk(y/n)?: ']); write = input(prompt,'s');
if strcmp(write,'y')
    cd(driveOut);  data = cell(6,size(Flow,3)/6);  mat_siz = size(Flow,1)*size(Flow,2);
...
%  REARRANGING & WRITING TO .DAT FILE
   for x = 1:6
     if m>1
        tmat = zeros(mat_siz*m+4,3);
        tmat(4:mat_siz+3,:)    = data{x,1};
        tmat(mat_siz+5:end,:)  = data{x,2};
        dlmwrite(type{x},tmat,'precision','%.5f','delimiter',' ');
     else
        tmat = zeros(mat_siz*m+3,3);
        tmat(4:mat_siz+3,:)    = data{x,1};
        dlmwrite(type{x},tmat,'precision','%.5f','delimiter',' ');
     end
   end 
%  LOGGING THE FILTERING PARAMETERS & IMAGE VECTOR COUNT
   removd = length(img_inf)+length(bad_U)+length(bad_V);      limits = zeros(4,2);
...
%  WRITING ALL THE DATA TO DISK LOCATION
   if strcmp(OutputStruct.nozzle,'Circular') || strcmp(OutputStruct.bckgrnd_root(1),'D')
...
%% SAVING MAT FILE
% drive_out = [root2 nozzle{1} '\' condition{1}(9:14) '\' condition{1}(1:7) '\'];
% if write == 1
%     save([drive_out 'Master_U'],'Master_U');   save([drive_out 'Master_V'],'Master_V');
%     save([drive_out 'X'],'X');      save([drive_out 'Y'],'Y');
% end  
%% STANDARD DEVIATOIN FILTERING 
% Algorithm: 1. Check image set at every location for outliers based on
% standard deviation criteria
%2. Record position of all non outlier images in to matrices for 
